...
%% External exports
%%====================================================================
-export ([compile/4, compile/2, sl_decode_term/2, sl_encode_term/2]).
...
generate_include (DestPath, FileName, Classes, _) ->
  ...
  {ok, IncludeFile} = file:open (DestPath ++ "/" ++ FileName ++ ".hrl",
                                 [write]),
...
override_property (Acc, [], NewProperty = #ontology_property{}) ->
...
generate_erlang (DestPath, FileName, Classes, NewClasses, _) ->
  ...
  io:format (ErlangFile, "-include (\"~s.hrl\").~n", [FileName]),
...
generate_sl_codec (DestPath, FileName, NewClasses, _) ->
  ...
  {ok, ConversionFile} = file:open (DestPath ++ "/" ++
                                    CodecName ++ ".erl", [write]),
  io:format (ConversionFile, "-module (~s).~n", [CodecName]),
  io:format (ConversionFile, "-include (\"acl.hrl\").~n", []),
  io:format (ConversionFile, "-include (\"~s.hrl\").~n",
             [FileName]),
  io:format (ConversionFile, "-export ([encode/1, decode/1]).~n~n",
             []),
...
%% Returns: [string()]
%%====================================================================
generate_sl_encoder_file (Classes) -> generate_sl_encoder_file ([], Classes).
...
generate_sl_encoder_file (Acc, [Class | T]) ->
  Head = io_lib:format ("encode(X) when is_record (X,'~s') ->~n  [\"~s\"",
...
%% Returns: [string()]
%%====================================================================
generate_sl_encoder_lines (Acc, _, []) -> lists:reverse (Acc);
...
generate_sl_encoder_lines (Acc, ClassName, [Property | T]) ->
  Line =
    io_lib:format (",~s",
                   [field_sl_encode (ClassName,
                                     Property#ontology_property.name,
...
%% Returns: [string()]
%%====================================================================
generate_sl_decoder_file (Classes) -> generate_sl_decoder_file ([], Classes).
...
%%   io:format ("~w~n", [OptionalPropertyLists]),
  ...
  OptionalPropertyLists = [Class#ontology_class.properties],

  Line = [generate_sl_decoder_clause (Class, X) || X <- OptionalPropertyLists],
...
%% Returns: [string()]
%%====================================================================
generate_sl_decoder_lines (Acc, _, []) -> lists:reverse (Acc);
...
generate_sl_decoder_lines (Acc, ClassName, [Property | T]) ->
  Line =
    io_lib:format ("~s,",
                   [sl_decode_term (ClassName,
                                 Property#ontology_property.name,
...
sl_decode_term (ClassName, FieldName, {set_of, _}, false, false) ->
  lists:flatten (
    io_lib:format ("'~s' = decode (set_of (sl:get_slot ('~s', T)))",
                   [FieldName, FieldName]));
sl_decode_term (ClassName, FieldName, {set_of, _}, true, false) ->
  lists:flatten (io_lib:format ("'~s' = set_of (sl:get_slot ('~s', T))",
                                [FieldName, FieldName]));

sl_decode_term (ClassName, FieldName, {sequence_of, _}, false, false) ->
  lists:flatten (
    io_lib:format ("'~s' = decode (sequence_of (sl:get_slot ('~s', T)))",
                   [FieldName, FieldName]));
sl_decode_term (ClassName, FieldName, {sequence_of, _}, true, false) ->
  lists:flatten (io_lib:format ("'~s' = sequence_of (sl:get_slot ('~s', T))",
                                [FieldName, FieldName]));

sl_decode_term (ClassName, FieldName, _, false, false) ->
  lists:flatten (io_lib:format ("'~s' = decode (sl:get_slot ('~s', T))",
                                [FieldName, FieldName]));
sl_decode_term (ClassName, FieldName, FieldType, true, false) ->
  lists:flatten (
    io_lib:format (
      "'~s' = ontology:sl_decode_term (sl:get_slot ('~s', T), ~s)",
...
sl_decode_term (ClassName, FieldName, _, false, true) ->
  lists:flatten (io_lib:format ("'~s' = decode (lists:nth (~w, T))",
                                [FieldName,
                                 digit_of (FieldName) + 1]));
sl_decode_term (ClassName, FieldName, FieldType, true, true) ->
  lists:flatten (
    io_lib:format (
      "'~s' = ontology:sl_decode_term (lists:nth (~w,T), ~w)",
      [FieldName, digit_of (FieldName) + 1, FieldType])).
