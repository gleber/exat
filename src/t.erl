...
-record(handler_state, {out = user, max_events = inf}).

-include_lib("stdlib/include/ms_transform.hrl").

ut() -> dbg:ctp().

t(Mod) when is_atom(Mod) -> t([Mod]);
t({Mod, Fun}) when is_atom(Mod), is_atom(Fun) -> t([{Mod, Fun}]);
t(What) -> t0(What, []).
...
ts(What) -> dbg:tracer(), dbg:p(all, [call]), t1(What, []).

t1([], _Flags) -> ok;
t1([{M, F} | T], Flags) -> dbg:tpl(M, F, Flags), t1(T, Flags);
t1([H | T], Flags) -> dbg:tpl(H, Flags), t1(T, Flags).
...
topcpu(Limit) -> toppi(Limit, reductions).

topmsgq() -> topmsgq(10).

topmsgq(Limit) -> toppi(Limit, message_queue_len).

topmem() -> topmem(10).

topmem(Limit) -> toppi(Limit, total_heap_size).

toppi(Limit, Info) -> toppi(processes(), Limit, Info).

toppi(Procs, Limit, Info) ->
    L = lists:flatmap(fun (P) ->
			      try [{P, element(2, erlang:process_info(P, Info))}] catch _:_ -> [] end
		      end,
		      Procs),
...
np(State) -> L = p(flashbot), n(State, L).

n(State, L) -> [oortle_info(P) || P <- L, state_is_not(State, P)].

state_is_not(State, P) ->
    {_, L} = process_info(P, dictionary),
    case lists:keyfind(oortle_state, 1, L) of
      {oortle_state, State} -> false;
      _ -> true
    end.
...
oortle_info(P) ->
    ...
    [{pid, P}, {state, State}, {in, lists:reverse(In)}, {out, lists:reverse(Out)}].

state(State) -> put(oortle_state, State).

save_packet(Key, Packet) when is_list(Packet) ->
    save_packet(Key, iolist_to_binary(Packet));
save_packet(Key, Packet) ->
    case get(Key) of
      undefined -> put(Key, [Packet]);
      [A, B, _] -> put(Key, [Packet, A, B]);
      L -> put(Key, [Packet | L])
    end.

last_in(Packet) -> save_packet(oortle_in, Packet).

last_out(Packet) -> save_packet(oortle_out, Packet).

get(P, K) -> {_, L} = process_info(P, dictionary), proplists:get_value(K, L).

pi(P) when is_pid(P) -> rpc:call(node(P), erlang, process_info, [P]).

msg_watch(P) -> spawn(fun () -> msg_watch1(P) end).
...
dec(#handler_state{max_events = 1} = _State) -> dbg:stop();
dec(#handler_state{max_events = N} = State) ->
    State#handler_state{max_events = N - 1}.

dhandler(end_of_trace, #handler_state{} = State) -> State;
dhandler(Trace, #handler_state{} = State)
...
dhandler(Trace, #handler_state{} = State)
    ...
    dhandler1(Trace, tuple_size(Trace) - 1, element(tuple_size(Trace), Trace),
	      State);
dhandler(Trace, #handler_state{out = Out} = State)
    when element(1, Trace) == drop, tuple_size(Trace) =:= 2 ->
    io:format(Out, "*** Dropped ~p messages.~n", [element(2, Trace)]), State;
dhandler(Trace, #handler_state{out = Out} = State)
    when element(1, Trace) == seq_trace, tuple_size(Trace) >= 3 ->
    SeqTraceInfo = case Trace of
		     {seq_trace, Lbl, STI, TS} ->
			 io:format(Out, "SeqTrace ~p [~p]: ", [TS, Lbl]), STI;
		     {seq_trace, Lbl, STI} -> io:format(Out, "SeqTrace [~p]: ", [Lbl]), STI
		   end,
    case SeqTraceInfo of
      {send, Ser, Fr, To, Mes} ->
	  io:format(Out, "(~p) ~p ! ~p [Serial: ~p]~n", [Fr, To, Mes, Ser]);
      {'receive', Ser, Fr, To, Mes} ->
	  io:format(Out, "(~p) << ~p [Serial: ~p, From: ~p]~n", [To, Mes, Ser, Fr]);
      {print, Ser, Fr, _, Info} ->
	  io:format(Out, "-> ~p [Serial: ~p, From: ~p]~n", [Info, Ser, Fr]);
      Else -> io:format(Out, "~p~n", [Else])
    end,
    dec(State);
dhandler(_Trace, #handler_state{} = State) -> State.
...
dhandler1(Trace, Size, #handler_state{out = Out} = State) ->
    ...
    case element(3, Trace) of
	  ...
	  case element(4, Trace) of
	    ...
	    MFA -> io:format(Out, "(~p) call ~s~n", [From, ffunc(MFA)])
	  end;
	  ...
	  case element(4, Trace) of
	    ...
	    MFA -> io:format(Out, "(~p) old_ret ~s~n", [From, ffunc(MFA)])
	  end;
      return_from ->
	  MFA = element(4, Trace),
	  Ret = element(5, Trace),
      ...
      Op -> io:format(Out, "(~p) ~p ~s~n", [From, Op, ftup(Trace, 4, Size)])
    end,
    dec(State).
...
dhandler1(Trace, Size, TS, #handler_state{out = Out} = State) ->
    ...
    case element(3, Trace) of
	  ...
	  case element(4, Trace) of
	    {dbg, ok} -> ok;
	    Message -> io:format(Out, "(~p) << ~p (Timestamp: ~p)~n", [From, Message, TS])
	  end;
      send ->
	  Message = element(4, Trace),
	  To = element(5, Trace),
	  ...
	  case element(4, Trace) of
	    MFA when Size == 5 ->
		Message = element(5, Trace),
		io:format(Out, "(~p) call ~s (~p) (Timestamp: ~p)~n",
			  [From, ffunc(MFA), Message, TS]);
	    MFA -> io:format(Out, "(~p) call ~s (Timestamp: ~p)~n", [From, ffunc(MFA), TS])
	  end;
      return -> %% To be deleted...
	  case element(4, Trace) of
	    MFA when Size == 5 ->
		Ret = element(5, Trace),
		io:format(Out, "(~p) old_ret ~s -> ~p (Timestamp: ~p)~n",
	  ...
	  MFA = element(4, Trace),
	  Ret = element(5, Trace),
	  io:format(Out, "(~p) returned from ~s -> ~p (Timestamp: ~p)~n",
		    [From, ffunc(MFA), Ret, TS]);
      return_to ->
	  MFA = element(4, Trace),
	  io:format(Out, "(~p) returning to ~s (Timestamp: ~p)~n",
		    [From, ffunc(MFA), TS]);
      spawn when Size == 5 ->
	  Pid = element(4, Trace),
	  MFA = element(5, Trace),
	  io:format(Out, "(~p) spawn ~p as ~s (Timestamp: ~p)~n",
		    [From, Pid, ffunc(MFA), TS]);
      Op ->
	  io:format(Out, "(~p) ~p ~s (Timestamp: ~p)~n",
...
ffunc({M, F, Arity}) -> io_lib:format("~p:~p/~p", [M, F, Arity]);
...
ftup(Trace, Index, Index) -> io_lib:format("~p", [element(Index, Trace)]);
