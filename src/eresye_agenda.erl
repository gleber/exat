...
handle_call({set_strategy, NewStrategy}, From,
            Agenda) ->
    {OldStrategy, RuleList, ExecutorPid, ExecState,
     NextId} =
        Agenda,
...
%%Returns the Id of activation associated to rule 'Rule' and
%%arguments 'Args', false if it not present
handle_call({get_activation, {Rule, Args}}, From,
            Agenda) ->
    %%Rule = {Module, Fun}, or
    %%Rulle = fun(...)
...
%%Returns the Id of first activation associated to rule 'Rule',
%%false if 'Rule' is not present in the agenda
handle_call({get_activation_from_name, Rule}, From,
            Agenda) ->
    {_, RuleList, _, _, _} = Agenda,
...
%%Return the salience value of activation with id='Id'
%%false if Id is not present in the agenda
handle_call({get_activation_salience, Id}, From,
            Agenda) ->
...
popRule([]) -> {false, []};
popRule([Activation | NewRuleList] = RuleList) ->
