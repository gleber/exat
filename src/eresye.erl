...
handle_call({add_rule, {Fun, Salience},
    ...
    case NConds of
        [] ->
            NewState = make_struct(State, Rule, PConds, [], Root,
                                   nil);
        _ ->
            ...
            Root1 = eresye_tree_list:refresh(Root,
                                             lists:nth(3, R1)),
            NewState = make_struct(R1, Rule, PConds, [], Root1,
                                   {Np_node, NConds})
    end,
    {reply, ok, NewState};
...
get_conds({Module, Func}, Ontology, ClauseID) ->
    ...
    case epp:parse_file(File2 ++ ".erl",
                        [".", "src", "include"], [])
    of
        {error, OpenError} ->
            io:format(">> parse_file error!!!~n~w:~w~n",
        ...
        {ok, Form} ->
            ...
            case search_fun(Form, Func, Records) of
                {error, Msg} ->
                    io:format(">> search_fun error!!!~n~w:~s~n",
                              [{Module, Func}, Msg]),
                    error;
                {ok, CL} ->
                    ...
                    case read_clause(SolvedClauses, [], Records) of
                        {error, Msg2} ->
                            io:format(">> read_clause error!!!~n~w:~s~n",
                                      [{Module, Func}, Msg2]),
                            error;
                        CondsList -> CondsList
                    end
...
search_fun([{function, _, Func, _, ClauseList} | Other],
           Func, RecordList) ->
    {ok, ClauseList};
...
read_clause([], CondsList, RecordList) -> CondsList;
...
get_record_def(Name, [{Name, Definition} | Rest]) ->
    Definition;
...
make_struct(R, Rule, [], P, Cur_node, ng) ->
    ...
    case eresye_tree_list:child(Key, Cur_node, Join) of
        false ->
            ...
            {Join2, Agenda1} = update_new_node(Node, Cur_node,
                                               Join1, Agenda);
        Node -> Join2 = Join, Agenda1 = Agenda
    end,
...
make_struct(R, Rule, [], P, Cur_node, {Nod, NConds}) ->
    ...
    case eresye_tree_list:child(Key, Cur_node1, Join2) of
        false ->
            ...
            {Join4, Agenda2} = update_new_node(Node, Cur_node2,
                                               Join3, Agenda1);
        Node ->
            {Fun, Salience} = Rule,
            Key1 = element(1, Node),
...
update_agenda(Agenda, [], Rule) -> Agenda;
...
retract_facts(R, FactMatch) ->
    ...
    lists:foldl(fun (X, S) -> retract_fact(S, X) end, R,
                Facts).

check_cond(R, [], {Fact, Sign}) -> R;
check_cond(R, [{C1, Tab, Alfa_fun} | T],
           {Fact, Sign}) ->
    case catch Alfa_fun(Fact) of
        ...
        Other -> check_cond(R, T, {Fact, Sign})
    end.
    ...
    case eresye_tree_list:get_key(Join_node1) of
        ...
        {Tab, Fun} ->
            Children_list = eresye_tree_list:children(Join_node1,
                                                      Join),
            {Join1, Agenda1} = pass_tok(Tok_list, Children_list,
                                        Join, Agenda)
    end,
...
left_act({Token, Sign}, [Join_node | T], Join,
    ...
    case Sign of
        plus -> Beta1 = Beta ++ [Token];
        minus -> Beta1 = Beta -- [Token]
    end,
...
refresh([], Join, L) -> lists:reverse(L);
